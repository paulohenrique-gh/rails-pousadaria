...
describe 'Room availability check API' do
  context 'GET api/v1/rooms/:room_id/check_availability' do
    it 'returns stay total when room is available' do
      ...
      # Act
      checkin = 10.days.from_now.to_date.strftime('%d-%m-%Y')
      checkout = 20.days.from_now.to_date.strftime('%d-%m-%Y')
      get(
    ...
    it 'fails when room is not found' do
      # Act
      get(
        "/api/v1/rooms/9999/check_availability/?checkin=bad request&checkout=2023/12/05"
      )
