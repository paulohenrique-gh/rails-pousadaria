...
describe 'User registers reservation' do
  it 'and is not authenticated' do
    ...
    # Act
    post(
      room_reservations_path(room.id),
      params: { checkin: 5.days.from_now, checkout: 15.days.from_now,
                guest_count: 2, stay_total: 900, room_id: room.id,
                guest_id: guest.id }
    )
  ...
  it 'without associated guest' do
    # Arrange
    user = User.create!(email: 'exemplo@mail.com', password: 'password')
    ...
    # Assert
    expect(response).to redirect_to new_room_reservation_path(room.id)
  end
end
